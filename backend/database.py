...
def newTable(cursor):
    cursor.execute("CREATE TABLE IF NOT EXISTS subst (date TEXT, class TEXT, hour NUMBER, teacher TEXT, room TEXT, notes TEXT)")

def createEntry(cursor, date, _class, hour, teacher, room, notes):
    cursor.execute("INSERT INTO subst VALUES (?, ?, ?, ?, ?, ?)", (date, _class, hour, teacher, room, notes))
